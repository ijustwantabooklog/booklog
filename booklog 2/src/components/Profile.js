                  ...
                  <div key={entry.id}
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ fontSize: 12, color: "#888" }}>
                        {entry._type === "book" ? entry.author : [entry.author, entry.publication].filter(Boolean).join(" · ")}
                      </div>
                    </div>
                  </div>
                ))}
          ...
          {allActivity.length > 0 && (
            <div style={cardStyle}>
              ...
              {allActivity.map((entry, i) => {
                ...
                const date = entry.createdAt?.toDate ? entry.createdAt.toDate().toLocaleDateString("en-US", { month: "short", day: "numeric" }) : "";
                return (
                  <div key={entry.id}
                    onClick={() => { if (isOwnProfile && id) isArticle ? onSelectArticle?.(id) : onSelectBook?.(id); }}
                    style={{ padding: "12px 16px", borderBottom: i === allActivity.length - 1 ? "none" : "0.5px solid #ebebeb", fontSize: 13, color: "#444", cursor: (isOwnProfile && id) ? "pointer" : "default", lineHeight: 1.5 }}
                    onMouseEnter={e => { if (isOwnProfile && id) e.currentTarget.style.background = "#fafafa"; }}
                    onMouseLeave={e => e.currentTarget.style.background = "none"}>
                    {entry.text} <strong style={{ color: "#333", fontWeight: 500 }}>{title}</strong>{entry.shelf ? ` "${entry.shelf}"` : ""}{entry.tag ? ` #${entry.tag}` : ""}{date ? ` · ${date}` : ""}
                  </div>
                );
              })}
            </div>
