              ...
              <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 6 }}>
                <textarea id="quote-text" value={newQuoteText} onChange={e => setNewQuoteText(e.target.value)}
                  onKeyDown={e => {
                    ...
                    if (e.key === "Tab") { e.preventDefault(); document.getElementById("quote-note")?.focus(); }
                  }}
                  placeholder="Quote"
                  rows={2}
                  style={{ ...bareInput, resize: "none", lineHeight: 1.5, fontSize: 14 }} />
                <input id="quote-note" value={newQuoteNote} onChange={e => setNewQuoteNote(e.target.value)}
